...
import { formatDimensions, getScoreColor, getScoreLabel } from "@/utils/photoProcessing";
...
export default function PhotoDetailScreen() {
  ...
  useEffect(() => {
    ...
    if (photo?.validationResults) {
      setTimeout(() => {
        scoreAnim.value = withSpring(photo.validationResults!.score / 100, {
          damping: 20,
          stiffness: 90,
        });
      }, 300);
    }
  }, [photo?.status, photo?.validationResults]);
  ...
  const handleShare = async () => {
    if (!photo?.processedUri) return;
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    try {
      await Share.share({
        message: `My passport photo for ${photo.countryName} - Generated by PassPic PRO`,
        url: photo.processedUri,
      });
    } catch (e) {
    ...
    <View style={[styles.container, { paddingBottom: bottomPad }]}>
      ...
      <View style={[styles.topBar, { paddingTop: topPad + 8 }]}>
        ...
        <Text style={styles.topBarTitle}>
          {isProcessing ? "Processing..." : "Passport Photo"}
        </Text>
        {isDone && (
          <Pressable
            onPress={handleShare}
          ...
          <>
            ...
            {photo.validationResults && (
              <Animated.View entering={FadeInDown.delay(200).springify()} style={styles.card}>
                ...
                <Text style={[styles.scoreLabel, { color: scoreColor }]}>
                  {getScoreLabel(photo.validationResults.score)}
                </Text>
              </Animated.View>
            )}
            ...
            {photo.validationResults && (
              <Animated.View entering={FadeInDown.delay(300).springify()} style={styles.card}>
                <Text style={styles.cardTitle}>Validation Checks</Text>
            ...
            {country && (
              <Animated.View entering={FadeInDown.delay(400).springify()} style={styles.card}>
                <Text style={styles.cardTitle}>Photo Specifications</Text>
                <View style={styles.divider} />
                {[
                  ...
                  { label: "Resolution", value: `${country.dpi} DPI` },
                  {
                    label: "Output Size",
                    value: `${country.widthPx}×${country.heightPx}px`,
                  },
                  { label: "Background", value: "White (#FFFFFF)" },
                  { label: "Notes", value: country.notes },
                ].map(({ label, value }) => (
            ...
            <Animated.View entering={FadeInUp.delay(450).springify()} style={styles.actions}>
              <Button
                title="Share Photo"
                onPress={handleShare}
